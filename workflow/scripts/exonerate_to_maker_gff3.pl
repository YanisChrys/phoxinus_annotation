...
sub process_gff_entry {
    ...
    if (@$segments_aref) {
    $MATCH_COUNTER++;
		my $type = ($MODE eq 'prot') ? "protein_match" : "match";
		my ($lend, $rend) = ($orient eq '+') ? ($segments_aref->[0]->[0], $segments_aref->[-1]->[1]) : ($segments_aref->[-1]->[0], $segments_aref->[0]->[1]);
		my $parentID = "$contig:$MATCH_COUNTER";
		print join("\t", $contig, "exonerate", "protein_match", $lend, $rend, ".", $orient, ".",
						   "ID=$parentID;Target=$target") . "\n";
		my $exon_number = 1;
	    foreach my $segment (sort { $a->[0] <=>  $b->[0] } @$segments_aref) {
	        my ($lend, $rend) = @$segment;
	        
	        my ($rel_info, $rel_pos, $rel_len, $rel_lend, $rel_rend);
			if ($rel_info = shift @$rel_coords_aref) {
		        ($rel_pos, $rel_len) = @$rel_info;
		        $rel_lend = $rel_pos;
		        $rel_rend = ($MODE eq 'prot') ? $rel_pos + $rel_len/3 : $rel_pos + $rel_len;
			} else {
				$rel_lend = $rel_rend = "-";
	        }
	
	        print join("\t", $contig, "exonerate", "match",
	                   $lend, $rend, $per_id, $orient,
	                   ".", "ID=$parentID:exon-$exon_number;Parent=$parentID;Target=$target $rel_lend $rel_rend") . "\n";
			$exon_number++;
	    }
	}
